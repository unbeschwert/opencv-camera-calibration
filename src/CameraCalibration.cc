...
void modifyImagePath(FS::path &image_path, std::string str_to_append) {
  ...
  std::string file_name =
      file_name_with_ext.substr(0, file_name_with_ext.find_last_of('.'));
  file_name_with_ext =
      file_name + (str_to_append + image_path.extension().string());
...
void CameraCalibration::fillObjectPoints(
    ...
    for (auto i = 0; i < calib_settings.board_size.height; i++)
      for (auto j = 0; j < calib_settings.board_size.width; j++)
        object_points_buffer.push_back(cv::Point3f(
            j * calib_settings.square_size, i * calib_settings.square_size, 0));
    break;
  }
  case (PatternType::CIRCLE_GRID): {
    for (auto i = 0; i < calib_settings.board_size.height; i++)
      for (auto j = 0; j < calib_settings.board_size.width; j++)
        object_points_buffer.push_back(
            cv::Point3f(j * calib_settings.dist_between_centers,
...
void CameraCalibration::findSalientPoints(
    const cv::Mat &image, std::vector<cv::Point2f> &salientPoints,
    FS::path &image_path) {
  cv::Mat image_copy;
  ...
  if (calib_settings.pattern == CHESS_BOARD) {
    ...
    bool found = cv::findChessboardCorners(
        cv::InputArray(image), calib_settings.board_size,
        cv::OutputArray(corners),
        cv::CALIB_CB_ADAPTIVE_THRESH + cv::CALIB_CB_NORMALIZE_IMAGE +
            cv::CALIB_CB_FAST_CHECK);
    if (found) {
      cv::cornerSubPix(cv::InputArray(image),
                       cv::InputOutputArray(salientPoints), cv::Size2i(11, 11),
                       cv::Size(-1, -1),
                       cv::TermCriteria(cv::TermCriteria::Type::EPS +
                                            cv::TermCriteria::Type::COUNT,
                                        30, 0.0001));
      cv::drawChessboardCorners(cv::InputOutputArray(image_copy),
                                calib_settings.board_size,
                                cv::InputArray(salientPoints), found);
      if (verbose) {
    ...
    if (calib_settings.pattern == CIRCLE_GRID)
      found = cv::findCirclesGrid(
          cv::InputArray(image), calib_settings.board_size,
          cv::OutputArray(salientPoints), cv::CALIB_CB_SYMMETRIC_GRID);
    else
      found = cv::findCirclesGrid(
          cv::InputArray(image), calib_settings.board_size,
          cv::OutputArray(salientPoints), cv::CALIB_CB_ASYMMETRIC_GRID);
    if (found) {
      cv::drawChessboardCorners(cv::InputOutputArray(image_copy),
                                calib_settings.board_size,
                                cv::InputArray(salientPoints), found);
...
void CameraCalibration::calibrateCamera(
    std::function<cv::Mat(void *)> getImageFromSDK, void *handle) {
  if (calib_settings.input == CAPTURED_IMAGES) {
    ...
    if (not FS::is_directory(image_folder))
      ...
      cv::calibrateCamera(
          ...
          cv::InputArrayOfArrays(image_points), calib_settings.board_size,
          cv::InputOutputArray(camera_matrix),
          cv::InputOutputArray(dist_coeffs), cv::OutputArrayOfArrays(rvecs),
          cv::OutputArrayOfArrays(tvecs), flags);
    }
...
// else use the FFMPEG library in linux
          ...
          cv::VideoCapture capture{video_path.string(), cv::CAP_DSHOW};
#else
          cv::VideoCapture capture{video_path.string(), cv::CAP_FFMPEG};
#endif
    ...
    if (not getImageFromSDK) {
      if (not device_id_is_set)
        std::cout
            << "device_id variable not set. Default value of 0 will be used";
    ...
    cv::calibrateCamera(
        cv::InputArrayOfArrays(object_points),
        cv::InputArrayOfArrays(image_points), calib_settings.board_size,
        cv::InputOutputArray(camera_matrix), cv::InputOutputArray(dist_coeffs),
        cv::OutputArrayOfArrays(rvecs), cv::OutputArrayOfArrays(tvecs), flags);
  } else {
...
double CameraCalibration::computeReprojectionErrors() {
  ...
  for (std::size_t i = 0; i < object_points.size(); i++) {
    ...
    projectPoints(cv::InputArray(object_points[i]), cv::InputArray(rvecs[i]),
                  cv::InputArray(tvecs[i]), cv::InputArray(camera_matrix),
                  cv::InputArray(dist_coeffs),
                  cv::OutputArray(projected_image_points));
    double error =
        cv::norm(cv::InputArray(image_points[i]),
                 cv::InputArray(projected_image_points), cv::NORM_L2);
    per_capture_projection_error[i] =
        std::sqrt((error * error) / points_per_capture);
